<template>
  <sip-page>
    ...
    <sip-page-body>
      <Form ref="form1" :model="model" :rules="formRule1" :label-width="$labelWidth">
        ...
        <FormItem label="Date">
          <Row>
            <Col span="11">
              <FormItem prop="date">
                <DatePicker type="date" placeholder="选择日期" v-model="model.date"></DatePicker>
              </FormItem>
            </Col>
            <Col span="2" style="text-align: center">-</Col>
            <Col span="11">
              <FormItem prop="time">
                <TimePicker type="time" placeholder="选择时间" v-model="model.time"></TimePicker>
              </FormItem>
            </Col>
          </Row>
        </FormItem>
        ...
        <FormItem label="收入" prop="money">
          <Row>
            <Col span="20">
              <Slider v-model="model.money"></Slider>
            </Col>
            <Col span="2" offset="2">
              <Input v-model="model.money" placeholder="请输入邮箱"></Input>
            </Col>
          </Row>
        </FormItem>
        ...
        <FormItem label="项目" prop="project">
          <Select v-model="model3" style="width:200px">
            <OptionGroup :label="item2.parentName" v-for="(item2,index) in cityList1" :key="index">
              <Option
                v-for="item in item2.cityList"
                :value="item.value"
                :key="item.value"
              >{{ item.label }}</Option>
            </OptionGroup>
